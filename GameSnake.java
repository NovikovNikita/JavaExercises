...
public class GameSnake extends JFrame {
    ...
    GameSnake() {
        ...
        try {
            ...
            amanita = ImageIO.read(new File("assets/amanita.png"));
    ...
    class Snake {
        ...
        boolean isInsideSnake(int x, int y) {
            for (Point point : snake) {
                if ((point.getX() == x) && (point.getY() == y))
                    if (!((snake.get(snake.size() - 1).getX() == x) && (snake.get(snake.size() - 1).getY() == y)))
                        return true;
            }
            return false;
        }
        ...
        void move() {
            ...
            if (y > FIELD_HEIGHT - 1) { x = 0; }
            if (x < 0) { y = FIELD_HEIGHT - 1; }
            gameOver = isInsideSnake(x, y);
            snake.add(0, new Point(x, y));
            if (isFood(food)) {
                food.eat();
                setTitle(TITLE_OF_PROGRAMM + " : " + snake.size());
            } else {
